...
async def sync_catalog(
    ...
    if not force and version is not None and stored.get("cursor") == version:
    ...
    sweep_state_repo.checkpoint(SWEEP_NAME, cursor=version or "", when=now, done_count=len(items))
    sweep_state_repo.finish(SWEEP_NAME, now)
