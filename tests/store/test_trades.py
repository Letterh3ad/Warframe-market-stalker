...
def _trade(side: Direction, qty: int, plat: int) -> Trade:
...
def test_record_and_read_back(conn):
    ...
    trade_id = repo.record(_trade(Direction.BUY, 3, 40))
    trades = repo.all_for("x", 0)
    assert trades[0].id == trade_id
    assert trades[0].side is Direction.BUY
    assert trades[0].quantity == 3


def test_holdings_view_nets_buys_against_sells(conn):
    repo = TradesRepo(conn)
    repo.record(_trade(Direction.BUY, 4, 40))
    repo.record(_trade(Direction.BUY, 2, 46))
    repo.record(_trade(Direction.SELL, 1, 60))
    assert repo.holdings() == [("x", 0, 5, 42.0)]


def test_fully_sold_positions_leave_holdings(conn):
    repo = TradesRepo(conn)
    repo.record(_trade(Direction.BUY, 2, 40))
    repo.record(_trade(Direction.SELL, 2, 60))
