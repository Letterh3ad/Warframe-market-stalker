...
# "due" on iteration 1 (see test_runner.py's fixture). The fixture below marks both
# done for START's date so the daily work stays out of a test about poll cadence.
    ...
    for slug, volume in (("hot", 400), ("cold", 6)):
        ...
        context.daily.upsert_many(
            [DailyCandle(slug=slug, rank=0, date=f"2026-06-{d:02d}",
                         close=50 + (d % 9 if slug == "hot" else 0), high=60, low=40,
...
async def test_a_simulated_day_respects_the_floor_the_ceiling_and_the_budget(ctx):
    report = await Daemon(ctx).run(max_iterations=600)
    ...
    # the volatile, liquid, moving item polls harder than the flat one
    assert counts["hot"] > counts["cold"]
    # nothing polls faster than the 2 minute ceiling
    ...
    for slug in ("hot", "cold"):
        ...
        # No single item is polled twice inside the ceiling.
