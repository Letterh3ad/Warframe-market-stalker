...
def test_excess_return_separates_an_item_from_its_cohort():
    context = build_context(SERIES, TAGS, days=7)
    features, samples = build(UP, tags=("mod",), context=context)
    assert features.tag == "mod"
    assert features.excess_return_7d == pytest.approx(
        returns_over(UP, 7) - context.tag_returns["mod"]
    )
    assert features.cohort_size == 2
    assert samples["market_cohort"] == 2


def test_an_item_whose_tag_has_no_cohort_falls_back_to_the_market():
    context = build_context(SERIES, TAGS, days=7)
    features, _ = build(UP, tags=("unknown_tag",), context=context)
    assert features.tag_median_return_7d == context.median_return
